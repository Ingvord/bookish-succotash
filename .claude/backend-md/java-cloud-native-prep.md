...
These recur whenever a role mentions Quarkus, Micronaut, or "cloud-native JVM," in the shape interviewers grade: the question, what they are really testing, the concise answer, then the trade-off. Lead with the mechanism.

**Why do Quarkus and Micronaut move work to build time?** *Testing whether you grasp the core thesis, not just the brand names.* The classic stacks do dependency injection, proxying, and configuration at runtime through reflection and classpath scanning, which runs on every startup, keeps metadata resident, and is invisible to ahead-of-time compilers. Doing that wiring at build time instead yields fast startup, a small memory floor, and code that GraalVM can compile to a native image.

**What is the closed-world assumption in native image, and why does it break reflection?** *Testing whether you understand the native-image constraint that defines the stack.* GraalVM statically analyzes everything reachable from `main` and discards the rest, so anything reached only reflectively, by dynamic proxy, or by loading a resource by name is invisible to the analysis, gets pruned, and fails at runtime. This is exactly why these frameworks exist in their current form: because they wire everything at build time, they know what is reflectively reachable and generate the native metadata for you.

**Will a native image always be faster than the JVM?** *Testing whether you resist hype and reason about workload lifecycle.* No. It starts in milliseconds and uses far less memory, but at steady state a JIT-warmed JVM can match or beat it on peak throughput, because the JIT profiles the running code and optimizes hot paths that ahead-of-time compilation must guess at. Native wins on cold start and density; the warmed JVM can win on sustained throughput.

**Quarkus or Micronaut, what is the real difference?** *Testing whether you can compare without declaring a false winner.* Both share the build-time thesis. Quarkus builds on familiar standards (CDI, JAX-RS, Hibernate) with an exceptional live-reload dev mode and the broader extension catalog. Micronaut wrote its own compile-time DI from scratch, giving the cleanest reflection-free story and turning missing-bean errors into compile errors. Neither is universally superior; pick on ecosystem fit and team familiarity.

**Explain the threading contract.** *Testing whether you know what runs on the event loop and what must not.* A handler returning a reactive type (`Uni`/`Multi` on Quarkus) is assumed non-blocking and runs on the I/O event loop, where you must never block; a handler returning a plain value runs on a worker thread where blocking is allowed. Override the inference with `@Blocking`/`@NonBlocking` on Quarkus or `@ExecuteOn` on Micronaut, and remember Quarkus detects many blocking calls on the I/O thread at runtime and throws, which is a guardrail.

**Uni versus Multi (or Mono versus Flux)?** *Testing basic reactive vocabulary fluency.* `Uni` carries a single asynchronous value or failure; `Multi` carries a stream of zero-to-many items with backpressure. Reactor's equivalents are `Mono` and `Flux`. Use `Uni` for one result (a lookup), `Multi` for a stream (server-sent events, a paged feed).

**What is the most common reactive bug?** *Testing whether you have actually shipped reactive code.* Forgetting that a `Uni`/`Multi` is lazy and cold: it describes a pipeline and does nothing until subscribed. A `Uni` you create but forget to return or subscribe to is a silent no-op, with no exception and no execution, which has no analogue in blocking code.

**How do you keep a reactive endpoint actually non-blocking?** *Testing whether you know reactive is all-or-nothing on the data path.* Use non-blocking data access end to end (Hibernate Reactive, the Vert.x reactive SQL client, or R2DBC) so nothing on the path blocks the event loop. A reactive endpoint that calls a blocking JDBC driver stalls every request on that loop and performs worse than plain blocking would; if you only have a blocking driver, mark the work `@Blocking` so it runs off the loop.

**When is full reactive worth it over virtual threads?** *Testing whether you reach for reactive by need, not by default.* Reach for reactive when you genuinely need streaming responses, real backpressure against an unsteerable firehose, or massive fan-out concurrency. For ordinary CRUD bottlenecked on the database, the reactive tax (cold-stream bugs, context loss, opaque stack traces) buys little, and on Java 21 virtual threads deliver the same I/O concurrency in readable blocking style.

---

## In the room: framing this at the interview

The single idea an interviewer wants to hear you articulate is the thesis these frameworks share: move dependency injection, configuration, and proxying from runtime to build time, which is what unlocks fast startup, a small memory floor, and GraalVM native images. If you can state that in one breath and then explain how it cascades (build-time wiring means no runtime reflection, which means native image works, which means millisecond cold starts and high replica density), you have demonstrated the mental model the rest of the conversation tests.

The two traps to avoid sound like overclaiming. Do not say native image is simply faster: be the candidate who says it wins on cold start and density while a JIT-warmed JVM can win on sustained peak throughput, and tie the choice to workload lifecycle. Do not declare Quarkus or Micronaut the winner: frame them as the same thesis with different ergonomics (standards and dev-mode versus bespoke compile-time DI) and pick on team fit. And when reactive comes up, show range by noting that on Java 21+ virtual threads cover most I/O concurrency in readable blocking style, so full reactive is for streaming, backpressure, and extreme fan-out, not a default. That blend of enthusiasm and restraint is exactly the senior signal.
