...
The single most important fact to state cleanly: as of Jakarta EE 9 the namespace migrated from `javax.*` to `jakarta.*`. This was a hard break caused by Oracle retaining the `javax` trademark. Any library, any tutorial, any Stack Overflow answer using `javax.servlet` or `javax.persistence` is pre-2020 and will not compile against a modern server. Jakarta EE 11 is the current release (it shipped in June 2025), aligned with Java 21, and it adds the Jakarta Data specification and CDI Lite on top of that namespace; Jakarta EE 12 is targeted for 2026 with a Java 21 source baseline and Java 25 at runtime. When you see a guide pinning the API at 10 or earlier, it predates the current platform.
...
These are the questions that recur for this stack, in the shape interviewers grade: the question, what they are really testing, the concise answer, then the catch. Answer in a sentence or two, lead with the mechanism, finish with the tradeoff.

**What actually changed between Java EE and Jakarta EE?** *Testing whether your knowledge is post-2020 and post-Oracle, not stale.* Governance moved from Oracle to the Eclipse Foundation, and because Oracle kept the `javax` trademark, Jakarta EE 9 renamed every API package from `javax.*` to `jakarta.*`. That namespace switch is a hard, compile-breaking change, so any code or tutorial using `javax.servlet` or `javax.persistence` predates 2020.

**When would you choose an application server over Spring Boot or a cloud-native framework?** *Testing whether you pick architecture by tradeoff, not by fashion.* When you have an existing EE estate and team fluency, a hard requirement for standards portability across vendors, or container-managed JTA spanning multiple datasources that you would otherwise hand-roll. Reach for Quarkus or Micronaut instead when startup time and memory footprint are first-class concerns, because a classic server's multi-second startup and large memory floor are liabilities under serverless or aggressive autoscaling.

**Explain CDI scopes.** *Testing whether you understand the proxy that makes injection across scopes work.* `@ApplicationScoped` is one instance for the whole app (the default for stateless services), `@RequestScoped` is one per HTTP request, `@SessionScoped` is one per user session and must be `Serializable`, and `@Dependent` binds the bean's life to whatever injected it. The subtlety is that injecting a narrower scope into a wider one works only because CDI injects a client proxy that resolves the real instance per call.

**Why can't a CDI bean be `final`?** *Testing whether you know injection is proxy-based, not magic.* Because the container injects a client proxy that subclasses the bean to do scope resolution and interception, and you cannot subclass a `final` class. The same proxying is why beans need a no-arg constructor.

**What is the N+1 problem and how do you fix it?** *Testing whether you can spot the most common ORM performance bug.* You load a list, then touch a lazy association per element, triggering one query per row on top of the original. Fix it with a `join fetch` or an entity graph that loads the association in the first query; do not make everything `EAGER`, which reintroduces N+1 globally.

**Why does `@Transactional` sometimes not start a transaction?** *Testing whether you understand interceptors run through proxies.* Self-invocation: when a method calls another `@Transactional` method on `this`, the call bypasses the proxy that applies the transaction interceptor, so no new transaction starts. Cross-bean calls go through the proxy and work; same-bean calls silently do not.

**TomEE versus plain Tomcat?** *Testing whether you know what the EE Web Profile actually adds.* Plain Tomcat is a servlet container (Servlets, JSP, WebSocket). TomEE adds the Jakarta EE Web Profile on top: CDI, JPA, JAX-RS, JTA, and JMS, so you get container-managed transactions and datasources on Tomcat's small, fast base without assembling a framework yourself.

**What breaks when you move work off the request thread?** *Testing whether you have actually debugged async context loss.* Everything thread-bound: the security principal, the JTA transaction, the CDI request scope, and the SLF4J MDC correlation ID all live in `ThreadLocal` and do not follow the work to another thread. You either propagate context explicitly or accept null principals, broken request-scoped lookups, and logs that lose their request ID.

**How do virtual threads change the application-server model?** *Testing whether you are current and know the pinning catch.* A blocking call on a virtual thread parks the virtual thread and frees its carrier OS thread, so the readable thread-per-request style scales to high concurrency without the ~200-thread pool ceiling. The catch is pinning: inside a `synchronized` block or a native call the virtual thread pins its carrier, so a hot `synchronized` path negates the benefit and should become a `ReentrantLock`.

**What is the difference between non-blocking I/O and a non-blocking algorithm?** *Testing whether you can separate two ideas that share a name.* Non-blocking I/O is about throughput: one thread multiplexes many connections through a selector so it never parks waiting on a socket. A non-blocking algorithm is about coordination: threads cooperate through compare-and-swap retry loops instead of locks, so no thread can block the others. Same adjective, unrelated problems.

**What is the ABA problem?** *Testing the depth of your lock-free understanding, a senior-only probe.* A compare-and-swap checks the current value, not its history, so if another thread changes a slot from A to B and back to A, your CAS succeeds even though the world moved underneath you. It is harmless for a counter but corrupts pointer-based lock-free structures; the fix is to version the reference with `AtomicStampedReference` so the stamp, not just the value, must match.

---

## In the room: framing this at the interview

What an interviewer is really probing with a Jakarta EE or TomEE candidate is not whether you can recite annotations; it is whether you understand the application-server model as a deliberate tradeoff and can say honestly when it stops paying off. Lead every answer with the mechanism, then name the catch, because the catch is where seniority shows. The three reframes that consistently land: the platform is a set of specifications with swappable vendor implementations (so "portability" is the point); almost everything is proxy-based (CDI scopes, `@Transactional`, why beans cannot be `final`), so the surprises all trace to the proxy; and the thread is a unit of context, so any async handoff loses the security, transaction, scope, and logging context bound to it.

When asked to compare with Spring Boot or the cloud-native JVM stacks, do not get defensive about the older platform: frame it by lifecycle. Existing EE estate, standards portability, or container-managed JTA across datasources favor the app server; startup time, memory density, and scale-to-zero favor Quarkus or Micronaut. Bring numbers (a multi-second startup, a few-hundred-megabyte floor) so the comparison is concrete, and close by noting that Java 21+ virtual threads let this stack reclaim high-concurrency I/O without a reactive rewrite, which is the modern through-line that signals you are current rather than maintaining a legacy mental model.
